...
<div class="container">
    ...
    <form method="POST" action="{{ route('vote.payment.verify') }}">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="payment_phone" name="payment_phone" required value="{{ old('payment_phone', $paymentPhone ?? '') }}">
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const ussd = @json($operator === 'orange' ? '*144*10*05690560*' . $amount . '#' : '*555*4*1*03301404*' . $amount . '#');
